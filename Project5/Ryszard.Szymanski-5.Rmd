...
## Zadanie 1
Przygotowano funckję `create_er_graph_adjacency_matrix`:
```{r, echo = FALSE}
create_er_graph_adjacency_matrix
```

Wygenerowano za jej pomocą losowy graf o 100 wierzchołkach:
```{r}
...
## Zadanie 10
Nie jest to zawsze możliwe np. $\{1, 3\}$. Myślę, że dobrym warunkiem na początek byłoby ograniczenie następujące ograniczenie sumy konfiguracji stopni przez liczbę możliwych krawędzi. Dodatkowo suma ta nie może być liczą nieparzystą -- każda krawędź ma dwa końce.


## Zadanie 11
Zaimplementowano funkcje `create_block_model_graph_adjacency_matrix` w celu oblicznaia macierzy incydencji w modelu blokowym
```{r}
...
**Wpływ parametrów na wyniki** im mniejsza różnica między prawdopodobieńtswami tym trudniej zobaczyć na wykresie konkretne społeczności.
...
Najkrótsza droga znaczące się zmniejsza podczas gdy współczynnik gronowania pozostaje wysoki -- jest to charakterystyczna cecha sieci małych światów. Niewielka liczba skrótów jest potrzeba do uzyskania efektu małych światów, przy jednoczesnym zachowaniu wysokiego współczynnika gronowania.
...
## Zadanie 14
Zaimplementowano algorytm Havela-Hakimiego
```{r}
